...
const stakeAddress = "0xcBA38601F0d0c5e2748e52A8AdC8E3a9Bacb7A26";

const tokenAddress = ["0xD9eB2A9a18a4A330316015899305801b533eeB29","0x0daaf67afe0ee5c868202da7d8a722fcd9981a37"];
...
async function balanceChecker(_tokenaddr,index){
    ...
    totalStakeAmount = window.web3.utils.fromWei(await window.StakeInstance.methods.balanceOf(_tokenaddr,address).call());
...
async function stake(){
    ...
    if(Number(userStakeAmount) > 0){
       $("#notifictionMessage").html("You have already Staked Once")
       $(".tipBox").css("opacity","1");
       return false;
    }
    a = window.web3.utils.toWei(a);
    window.StakeInstance.methods.setAnnualMintPercentage(userStakedTokenAddess,mintpercent).send({from: address,value: 0,})
    .on('transactionHash', (hash) => {
