...
interface minter{
    ...
    function transferMintingOwnerShip(address _whom) external returns (bool);
}
...
contract staking is Paynodes {
    ...
    function stake(address _token,uint256 _amount) external returns(bool){
        ...
        if (lastStakeClaimed[_token][msg.sender] == 0) {
            ...
            minter(_token).mintToken(_stakeReward);
            lastStakeClaimed[_token][msg.sender] = now;
    ...
    //accepting Minting OwnerShip
    function acceptMintingOwnerShip(address _token) external onlyOwner() returns(bool){
        require(listedToken[_token],"ERR_TOKEN_IS_NOT_LISTED");
        minter(_token).acceptMintingOwnerShip();
        return true;
    }
    
    //transferring Minting OwnerShip
    function transferMintingOwnerShip(address _token,address _whom) external onlyOwner() returns(bool){
        require(listedToken[_token],"ERR_TOKEN_IS_NOT_LISTED");
        minter(_token).transferMintingOwnerShip(_whom);
        return true;
    }
